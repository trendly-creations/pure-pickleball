...
(function ($) {
    ...
    $_webnukes.videos = videos = {
		...
		dailymotion: function (url) {
            var settings = {'source': 'dailymotion', 'id': 'id', 'thumb': 'thumbnail_large_url', 'title': 'title', 'desc':'description', 'duration':'duration', 'tags':'tags', 'athor':'owner'};
            var videoInfo = url.match(/^.+dailymotion.com\/(video|hub)\/([^_]+)[^#]*(#video=([^_&]+))?/);
            /** Error message if the URL is not correct */
            if (!$('#dailymotion-' + videoInfo[2]).length) /** Stop Duplicates */
            {
                this.log('<strong>Fetching video info:</strong> ' + url);
                $.getJSON('https://api.dailymotion.com/video/' + encodeURIComponent(videoInfo[2]) + '?fields=id,thumbnail_large_url,title,duration,tags,description,owner', function (data) {
                    $.each(settings, function (sk, sv) {
        ...
        reset: function (logs) {
            ...
            else $('#' + this.options.content).sortable({containment: "#video-gallery", handle: '.move_upload_img'});
            /** sortable */
...
jQuery(document).ready(function ($) {
    ...
    $('.upload_gallery').on('click', 'i.play_upload_img, i.edit_upload_img', function (e) {
        ...
        } else dialog.create = function (event, ui) {
