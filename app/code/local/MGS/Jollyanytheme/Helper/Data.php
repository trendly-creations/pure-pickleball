...
class MGS_Jollyanytheme_Helper_Data extends Mage_Core_Helper_Abstract
{
	public function getCol(){
		...
		if(Mage::registry('current_category')){
			...
			if($_current_category->getPageLayout()!=''){
				$pageLayout = $_current_category->getPageLayout();
	...
	public function getFeaturedProducts(){
		if($featuredAttribute = Mage::getStoreConfig('jollyanytheme/general/featured')){
	...
	public function getBestsellingProducts(){
		if($bestsellingAttribute = Mage::getStoreConfig('jollyanytheme/general/bestselling')){
			$products = Mage::getModel('catalog/product')
				->getCollection()
				->addAttributeToSelect(array('name','small_image', 'final_price', 'price', 'special_price', 'product_label'))
				->addAttributeToFilter(Mage::getStoreConfig('jollyanytheme/general/bestselling'),1)
				->addAttributeToFilter('status',1)
				->addAttributeToFilter('visibility',array('neq'=>1))
				->addAttributeToFilter('type_id', array('neq'=>'virtual'))
				->setPageSize(5);
	...
	public function getCategoriesOfProduct($product){
		...
		$nameString = implode(', ',$html);
		return $nameString;
	}
	
	public function getAccountUrl(){
		$html = '<span class="topbar-login"><i class="fa fa-user"></i> ';
		
		if(Mage::getSingleton('customer/session')->isLoggedIn()){
			$html.= '<a href="'.Mage::getUrl('customer/account').'">'.$this->__('My Account').'</a>
				<span class="dropdown-links box">
					<a href="'.Mage::getUrl('wishlist').'"><span class="fa fa-heart"></span>&nbsp;&nbsp;'.$this->__('My Wishlist').'</a>
					<a href="'.Mage::getUrl('customer/account/logout').'"><span class="fa fa-lock"></span>&nbsp;&nbsp;&nbsp;'.$this->__('Logout').'</a>
				</span>
			';
		}
		else{
			$html.= '<a href="'.Mage::getUrl('customer/account').'">'.$this->__('Login / Register').'</a>';
		}
	...
	public function getSliderHtml($_item){
		$html = strip_tags($this->getLayout()->createBlock('cms/block')->setBlockId($_item->getIdentifier())->toHtml(),'<img>');
		$html = str_replace('<img', '<img data-bgfit="cover" data-bgposition="left top" data-bgrepeat="no-repeat" alt="slidebg1"', $html);
		return $html;
	}
	
	public function getCustomServicesBackground(){
		$img = strip_tags($this->getLayout()->createBlock('cms/block')->setBlockId('jollyany-custom-services-background')->toHtml(), '<img>');
		$arr = explode('src="',$img);
		
		if(isset($arr[1])){
			$srcArr = explode('"',$arr[1]);
			if(isset($srcArr[0])){
				return $srcArr[0];
			}
		}
	...
	public function getOurBrandBackground(){
		$img = strip_tags($this->getLayout()->createBlock('cms/block')->setBlockId('jollyany-our-brands-background')->toHtml(), '<img>');
		$arr = explode('src="',$img);
	...
	public function getSaleProducts(){
		...
		if(Mage::getStoreConfig('jollyanytheme/general/hot')!=''){
			$products = Mage::getModel('catalog/product')
				->getCollection()
				->addAttributeToSelect(array('name','small_image', 'final_price', 'price', 'special_price', 'product_label'))
				->addAttributeToFilter('product_label',Mage::getStoreConfig('jollyanytheme/general/hot'))
				->addAttributeToFilter('status',1)
				->addAttributeToFilter('visibility',array('neq'=>1))
				->addAttributeToFilter('type_id', array('neq'=>'virtual'));
			return $products;
	...
	public function getNewProducts(){
		...
		if(Mage::getStoreConfig('jollyanytheme/general/new')!=''){
			$products = Mage::getModel('catalog/product')
				->getCollection()
				->addAttributeToSelect(array('name','small_image', 'final_price', 'price', 'special_price', 'product_label'))
				->addAttributeToFilter('product_label',Mage::getStoreConfig('jollyanytheme/general/new'))
				->addAttributeToFilter('status',1)
				->addAttributeToFilter('visibility',array('neq'=>1))
				->addAttributeToFilter('type_id', array('neq'=>'virtual'));
			return $products;
	...
	public function relativeTime($pastTime){
		...
		if($difference < 3600)			return intval($difference/60)." minutes ago";
		if($difference <= 1.5*3600) 		return "One hour ago";
		if($difference < 23.5*3600)		return round($difference/3600)." hours ago";
		if($difference < 1.5*24*3600)           return "One day ago";
		if($difference < 8640000000)		return  round($difference/86400)." days ago";
	...
	public function getLastTwitter(){
		$tweetUser = Mage::getStoreConfig('jollyanytheme/twitter/user');
		$token = Mage::getStoreConfig('jollyanytheme/twitter/token');
		$token_secret = Mage::getStoreConfig('jollyanytheme/twitter/token_secret');
		$consumer_key = Mage::getStoreConfig('jollyanytheme/twitter/consumer_key');
		$consumer_secret = Mage::getStoreConfig('jollyanytheme/twitter/consumer_secret');
		$count = Mage::getStoreConfig('jollyanytheme/twitter/count');
		...
		if($token!='' && $token_secret!='' && $consumer_key!='' && $consumer_secret!='' && $tweetUser!=''){
			if(!$twitter_data->errors[0]->message){
				try{
					if(count($twitter_data)>0){
						foreach ($twitter_data as $key => $value) {
							$html .= '<li><span></span><p>'.$this->formatTwitString($twitter_data[$key]->text).' <a href="https://twitter.com/'.$twitter_data[$key]->user->screen_name.'/status/'.$twitter_data[$key]->id_str.'">about '.$this->relativeTime($twitter_data[$key]->created_at).'</a></p></li>';
						}
					}
				}
				catch(Exception $e){
	...
	public function getPageContent($url){
		...
		foreach($html->find('div#product-grid-container') as $content){
			$result['maincontent'] = $content->innertext;
		}
		
		$result['maincontent'] = str_replace('class="shop_item"','class="shop_item" style="display:none"',$result['maincontent']);
		
		foreach($html->find('span#number-to') as $amount){
	...
	public function getFirstImageUrl($content){
		...
		if(isset($matches[1][0])){
			...
			$html = '<div class="entry"><img class="img-responsive" alt="" src="'.$first_img.'"/></div>';
		}
		return $html;
	}
}
